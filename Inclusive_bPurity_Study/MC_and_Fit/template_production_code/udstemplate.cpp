...
//  chain1->Add("../QCD_Pt15-1000_das_sample/*.root");
  ...
  TFile * file2 = new TFile("../../templates/uds_template_QCD_Pt-15to300_10JetpTcut.root","RECREATE");

  TH1F * HgenMuB_pt=new TH1F("HgenMuUDS_pt","Generated muons p_{T};p_{T} [GeV];entries/bin",70,pt_cut-0.2,100.);
  TH1F * HgenMuB_eta=new TH1F("HgenMuUDS_eta","Generated muons #eta^{gen};#eta^{gen};entries/bin",70,-eta_cut-0.8,eta_cut+0.8);
...
*/
 ...
 // TH1F * Hpt_rel_gen_vs_reco = new TH1F("Hpt_rel_gen_vs_reco","p_{T}^{rel}(gen) -vs- p_{T}^{rel}(reco);p_{T}^{rel} [GeV];entries/bin",60,0.,4.5);
  ...
  TH1F *Hpt_rel_range1 = new TH1F("Hpt_rel_range_9to12",("p_{T}^{rel}(uds#rightarrow #mu +X): "+std::to_string((int)min_1)+"#leq p_{T}(#mu)#leq"+std::to_string((int)max_1)+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
  TH1F *Hpt_rel_range2 = new TH1F("Hpt_rel_range_12to17",("p_{T}^{rel}(uds#rightarrow #mu +X): "+std::to_string((int)max_1)+"<p_{T}(#mu)#leq"+std::to_string((int)max_2)+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
  TH1F *Hpt_rel_range3 = new TH1F("Hpt_rel_range_17toInf",("p_{T}^{rel}(uds#rightarrow #mu +X): "+std::to_string((int)max_2)+"<p_{T}(#mu)"+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);

  TH1F *Hpt_rel_range1_no_dif = new TH1F("Hpt_rel_range_9to12_no_dif",("p_{T}^{rel}(uds#rightarrow #mu +X): "+std::to_string((int)min_1)+"#leq p_{T}(jet)#leq"+std::to_string((int)max_1)+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
  TH1F *Hpt_rel_range2_no_dif = new TH1F("Hpt_rel_range_12to17_no_dif",("p_{T}^{rel}(uds#rightarrow #mu +X): "+std::to_string((int)max_1)+"<p_{T}(jet)#leq"+std::to_string((int)max_2)+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
  TH1F *Hpt_rel_range3_no_dif = new TH1F("Hpt_rel_range_17toinf_no_dif",("p_{T}^{rel}(uds#rightarrow #mu +X): "+std::to_string((int)max_2)+"<p_{T}(jet)"+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
 ...
 //2.04.19: Lets check the flactuation of pT_rel distribution due to different pT(Jet) bins. Ideally there should be none due to invariance of pT_rel under boosts along the momentum axis of the quark. However, pT(Jet) != pT(quark) most of the times. Also not symmetric around pT(quark)/pT(jet)=1.
  ...
  TH1F *Hpt_rel_jet_range1 = new TH1F("Hpt_rel_jet_25to75",("p_{T}^{rel}(uds#rightarrow #mu+X): "+std::to_string((int)jet_min_1)+"#leq p_{T}(#mu)#leq"+std::to_string((int)jet_max_1)+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
  TH1F *Hpt_rel_jet_range2 = new TH1F("Hpt_rel_jet_75to150",("p_{T}^{rel}(uds#rightarrow #mu+X): "+std::to_string((int)jet_max_1)+"#leq p_{T}(#mu)#leq"+std::to_string((int)jet_max_2)+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
  TH1F *Hpt_rel_jet_range3 = new TH1F("Hpt_rel_jet_150toInf",("p_{T}^{rel}(uds#rightarrow #mu +X): "+std::to_string((int)jet_max_2)+"< p_{T}(#mu)"+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);

  TH1F *Hpt_rel_jet_range1_no_dif = new TH1F("Hpt_rel_jet_25to50_no_dif",("p_{T}^{rel}(uds#rightarrow #mu+X): "+std::to_string((int)jet_min_1)+"#leq p_{T}(jet)#leq"+std::to_string((int)jet_max_1)+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
  TH1F *Hpt_rel_jet_range2_no_dif = new TH1F("Hpt_rel_jet_50to75_no_dif",("p_{T}^{rel}(uds#rightarrow #mu+X): "+std::to_string((int)jet_max_1)+"#leq p_{T}(jet)#leq"+std::to_string((int)jet_max_2)+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);
  TH1F *Hpt_rel_jet_range3_no_dif = new TH1F("Hpt_rel_jet_75toInf_no_dif",("p_{T}^{rel}(uds#rightarrow #mu +X): "+std::to_string((int)jet_max_2)+"< p_{T}(jet)"+";p_{T}^{rel} [GeV];entries/bin").c_str(),60,0.,6.5);


  TTree tree1("output_matching", "output_matching");
vector<float> GENMUUDS_PT,GENJET_PT;//for pt(mu)-vs-pt(jet) and pt(mu)-vs-pt(jet).
...
vector<float>  GENMuUDS_pt,GENMuUDS_eta,GENMuUDS_phi,GENpt_rel,pt_rel_diff,GEN_jet_mu_DR;
  //recoJet= reconstructed jet that got matched with a genJet 
   ...
   tree1.Branch("GENMuUDS_pt",&GENMuUDS_pt);
   tree1.Branch("GENMuUDS_eta",&GENMuUDS_eta);
   tree1.Branch("GENMuUDS_phi",&GENMuUDS_phi);
...
//   cout<<"Current File Name: "<<current_input_file<<endl;
  ...
  //  cout<<"=========START OF EVENT========="<<endl;
   ...
   GENMuUDS_pt.clear(),GENMuUDS_eta.clear(),GENMuUDS_phi.clear(),GENpt_rel.clear(),pt_rel_diff.clear(),GEN_jet_mu_DR.clear();//Initialized outside of this scope in order to be written it in the tree1 tree...
    ...
    float px_mu,py_mu,pz_mu,pt_rel,px_jet,py_jet,pz_jet, m_mu=0.106,mt;
     ...
     TVector3 p_jet;  
       ...
       //Filling vectors from the leafs of *.root files, which are generated by TriggerAnalyzer.cc (Giorgos Karathanasis)
          for(int j=0; j<MC.genMuUDS_pt->size(); j++){

            if(MC.genMuUDS_pt->size()==0) continue;
            if(MC.genMuUDS_pt->at(j)<=pt_cut || abs(MC.genMuUDS_eta->at(j))>=eta_cut) continue;//Kinematic CUTS                                          
                  
              GENMuUDS_pt.push_back(MC.genMuUDS_pt->at(j));
              GENMuUDS_eta.push_back(MC.genMuUDS_eta->at(j));
              GENMuUDS_phi.push_back(MC.genMuUDS_phi->at(j));

                                                              }//end_for GENMuUDS_* leaf-vector size.
          if(GENMuUDS_pt.size()==0) continue;
...
//===================================genJet-recoJet matching=============================================================
 ...
 if(abs(MC.genJet_flavour->at(igen_jet)) ==4 || abs(MC.genJet_flavour->at(igen_jet)) ==5) continue; //choosing uds-Jets.
...
//TGraph
  for(int igen_mu=0;igen_mu<GENMuUDS_pt.size();++igen_mu){
     ...
     //    if(abs(MC.genJet_eta->at(ijet))>=jet_eta_cut||) continue;
         ...
         if(DR(genJet_eta.at(ijet),genJet_phi.at(ijet),GENMuUDS_eta.at(igen_mu),GENMuUDS_phi.at(igen_mu))>=deltaR) continue;
          deltaR= DR(genJet_eta.at(ijet),genJet_phi.at(ijet),GENMuUDS_eta.at(igen_mu),GENMuUDS_phi.at(igen_mu));
          genjet_mu_match=ijet;
                       }
            if(deltaR>=0.4) continue; 
            GEN_jet_mu_DR.push_back(deltaR);
            GENMUUDS_PT.push_back(GENMuUDS_pt.at(igen_mu)); GENJET_PT.push_back(genJet_pt.at(genjet_mu_match));//for pt(mu)-vs-pt(jet) and pt(mu)-vs-pt(jet).

            genmu_jet_map[igen_mu]=genjet_mu_match;
            //cout << igen_mu <<"===>"<<genmu_jet_map[igen_mu]<<", with deltaR= "<<deltaR<<endl;
                   
             mt= TMath::Sqrt(GENMuUDS_pt.at(igen_mu)*GENMuUDS_pt.at(igen_mu)+m_mu*m_mu) ;              
             px_mu = GENMuUDS_pt.at(igen_mu) * cos(GENMuUDS_phi.at(igen_mu));
             py_mu = GENMuUDS_pt.at(igen_mu) * sin(GENMuUDS_phi.at(igen_mu));
             pz_mu = mt * sinh(GENMuUDS_eta.at(igen_mu));          
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    genmuons=GENMuUDS_pt.size();   // cout << "yes and genmuons= " << genmuons << endl;  //Just for debugg.
...
/*
                ...
                HgenMuUDS_pt->Fill(GENMuUDS_pt.at(k));
                HgenMuUDS_eta->Fill(GENMuUDS_eta.at(k));
...
//--------------------------------------------------------RECO-GEN MATCHING----------------------------------------------------------
   ...
   map <int, int> gen_reco_map; //dynamic  pairing basically a "vector<pair<int,int>>".
            ...
            for(int q=0;q<genmuons;q++){
                ...
                gen_phi=GENMuUDS_phi.at(q);
                gen_eta=GENMuUDS_eta.at(q);
...
//07.04.19:
           ...
           //some_string.find('a') will EITHER return the position of the first appearance of 'a' in some_string if 'a' appears at least once in some_string, OR it will return a value that is equal to
           //std::string::npos if 'a' does not appear at all in some_string.
                  temp_diff = (GENMuUDS_pt.at(it->first)-RECOMu_unmatched_pt.at(it->second))/GENMuUDS_pt.at(it->first);
                  Hmu_reco_vs_gen_pt->Fill(temp_diff,relative_weight_30to50);
                  temp_diff = GENMuUDS_eta.at(it->first)-RECOMu_unmatched_eta.at(it->second);
                  Hmu_reco_vs_gen_eta->Fill(temp_diff,relative_weight_30to50); 
                  temp_diff = (GENMuUDS_phi.at(it->first)-RECOMu_unmatched_phi.at(it->second));
...
//====================================p_T(rel) calculation=============================================================
           ...
           for(int m=0; m<recomuons;m++){
             ...
             if(RECOJet_pt.size() < RECOMu_pt.size()){ 
                   ++pls;
                                 continue;
...
vector<float> RATIO;
 for(int i=0;i<GENJET_PT.size();++i) RATIO.push_back(GENMUUDS_PT[i]/GENJET_PT[i]);

const TVectorF genMuUDS_pt_genJet_pt_ratio_yAxis(RATIO.size(), &RATIO[0]);
const TVectorF genJet_yAxis(GENJET_PT.size(), &GENJET_PT[0]);
const TVectorF genMuUDS_xAxis(GENMUUDS_PT.size(), &GENMUUDS_PT[0]);

TGraph * genMuUDS_pt_vs_genJet_pt = new TGraph(genMuUDS_xAxis,genJet_yAxis);
         genMuUDS_pt_vs_genJet_pt->SetTitle("genMu_pt -vs- corresponding GenJet");
         genMuUDS_pt_vs_genJet_pt->GetXaxis()->SetTitle("p_{T}^{muon} [GeV]");
         genMuUDS_pt_vs_genJet_pt->GetYaxis()->SetTitle("p_{T}^{jet} [GeV]");

TGraph * genMuUDS_pt_genJet_pt_ratio = new TGraph(genMuUDS_xAxis,genMuUDS_pt_genJet_pt_ratio_yAxis);
         genMuUDS_pt_genJet_pt_ratio->SetTitle("genMu_pt/GenJet_pt");
         genMuUDS_pt_genJet_pt_ratio->GetXaxis()->SetTitle("p_{T}^{muon} [GeV]");
         genMuUDS_pt_genJet_pt_ratio->GetYaxis()->SetTitle("p_{T}^{muon}/p_{T}^{jet}");
...
TCanvas * finding_pt_jet_cut = new TCanvas("finding_genJet_pt_cut");
         ...
         genMuUDS_pt_vs_genJet_pt->Draw("AP");
         finding_pt_jet_cut->cd(2);
         genMuUDS_pt_genJet_pt_ratio->Draw("AP");
